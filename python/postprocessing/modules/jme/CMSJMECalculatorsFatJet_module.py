...
class CMSJMECalculatorsFatJet(Module):
    def __init__(self):
    ...
    def analyze(self, event):
        ...
        config = calcConfig.FatJetVariations()
        configureCalc(config, jetType="AK8PFPuppi", jerTag="Summer22EEPrompt22_JRV1_MC", splitJER=False,
                jecTag="Summer22EE_22Sep2023_V2_MC", levels=["L1FastJet", "L2Relative", "L3Absolute"],
                uncSources=["Total"],
                jecDBCache=self.jecDBCache, jrDBCache=self.jrDBCache)        
        configureFatJetCalc(config, isMC=True, year="2022", doSmearing=True, isTau21DDT=False,
                    jecDBCache=self.jecDBCache, jrDBCache=self.jrDBCache)
        loadJMESystematicsCalculators()
        jer = config.create()

        res = jer.produce(*getFatJetArgs(event._tree, isMC=True, addHEM2018Issue=False))
        print(jer.available())
