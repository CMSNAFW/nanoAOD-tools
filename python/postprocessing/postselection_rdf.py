...
################### utils ###################
def cut_string(cut):
    return cut.replace(" ", "").replace("&&","_").replace(">","g").replace(".","_").replace("==","e")
...
def makehistos(samples, var, cut):
    ...
    for s in samples.keys():
        ...
        '''
        nfiles_max = 3
        ntot = np.sum(samples[s]['ntot'][:nfiles_max])
        print('Ntot =', ntot)
        w = sample_dict[s].sigma*lumi/ntot
        if opt.recreate :
            ofile = ROOT.TFile.Open(repohisto+s+'.root', "RECREATE")
        else:
            ofile = ROOT.TFile.Open(repohisto+s+'.root', "UPDATE")
        df = ROOT.RDataFrame("Events", samples[s]['strings'][:nfiles_max])
        ...
        for v in var:
            h = (df_plot.Histo1D((v._name+"_"+s_cut," ;"+v._title+"", v._nbins, v._xmin, v._xmax), v._name).GetValue())
            h.Scale(w)
            h.Write()
        ofile.Close()
        print("Done "+s+" !")
...
if opt.plots:
    ...
    for d in datasets:
        ...
        makehistos(samples[d.label], var, c)
