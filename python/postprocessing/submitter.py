...
import sys
import time
from samples.samples_fra import *
...
def sub_writer(path, dat_name, outname, label):
    ...
    f.write("+Tag                    = '"+label+"'\n") # options are espresso = 20 minutes, microcentury = 1 hour, longlunch = 2 hours, workday = 8 hours, tomorrow = 1 day, testmatch = 3 days, nextweek     = 1 week
    f.write("executable              = runner_fra.sh\n")
...
'''
datasets = [TT_hadr_2022,TT_semilep_2022,ZJetsToNuNu_HT1500to2500_2022 ]
#datasets = [ZJetsToNuNu_HT800to1200_2018]

for d in datasets:
    dat = get_files_string(d)[1]
    dat = 'root://cms-xrd-global.cern.ch//'+dat
    outname=dat.split("/")[-1].replace('.root', '_output.root')
    final_name = '/eos/user/f/fscrivan/datasets/'+d.label+'_Skim.root'
    label = d.label
