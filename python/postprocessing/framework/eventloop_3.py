...
class Module(object):
    ...
    def beginJob(self, histFile=None, histDirName=None):
        ...
        if histFile and histDirName:
            ...
            self.dir = self.histFile.Get(histDirName)
            if not self.dir:
                print(f"[WARNING] Directory '{histDirName}' not found in histFile, using histFile directly.")
                self.dir = self.histFile

            self.dir.cd()
            self.objs = []

    def endJob(self):
        if self.writeHistFile and hasattr(self, 'objs'):
            self.dir.cd()
