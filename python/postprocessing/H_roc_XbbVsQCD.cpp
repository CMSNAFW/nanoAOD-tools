...
if(nentries>60000) nentries=60000;
  for(int i=0; i<nentries;i++){
    ...
    for(int k=0; k<nFatJet;k++){
      ...
      if(isH){
        nH_true=nH_true+1;
        if(FatJet_particleNetMD_Xbb[k]/(FatJet_particleNetMD_QCD[k]+FatJet_particleNetMD_Xbb[k])>=0.88) true_PN->Fill(FatJet_msoftdrop[k]);
        ...
        for(int u=0;u<101;u++){
        if( FatJet_particleNetMD_Xbb[k]/(FatJet_particleNetMD_QCD[k]+FatJet_particleNetMD_Xbb[k])>cut_PN[u]){ 
        ...
        if(FatJet_particleNetMD_Xbb[k]/(FatJet_particleNetMD_QCD[k]+FatJet_particleNetMD_Xbb[k])>=0.88) false_PN->Fill(FatJet_msoftdrop[k]);
...
l2->Draw();
c2->SaveAs("rocH_cutmass_XbbvsQCD.C");
