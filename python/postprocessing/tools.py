...
"""
vecchia definizione di good jet e fatjet
def get_jet(jets):
    
    #return list(filter(lambda x : x.jetId and x.pt>25 , jets))
    return list(filter(lambda x : x.pt>25 , jets))
...
#definizione di Leo
def is_good_Jet(jet):
    if jet.jetIdTightLeptonVeto == 0 and jet.jetIdTight == 0: return False
    if jet.pt < 30: return False
    if abs(jet.eta) > 2.4: return False
    return True        
def get_jet(jets):
    
    return list(filter(lambda x: is_good_Jet(x), jets))



def is_good_FatJet(fatjets):
    if fatjet.fatjetIdTightLeptonVeto== 0 and fatjet.fatjetIdTight == 0: return False
