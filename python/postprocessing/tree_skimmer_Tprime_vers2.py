...
def reco(scenario, isMC, addPDF, training):
    ...
    FatJet_pt1_nominal = array.array('f',[0.])
    FatJet_eta1_nominal = array.array('f',[0.])
    FatJet_phi1_nominal = array.array('f',[0.])
    FatJet_M1_nominal = array.array('f',[0.])
    ...
    systTree.branchTreesSysts(trees, scenario,"FatJet_pt1_nominal", outTreeFile, FatJet_pt1_nominal)
    systTree.branchTreesSysts(trees, scenario,"FatJet_eta1_nominal", outTreeFile, FatJet_eta1_nominal)
    systTree.branchTreesSysts(trees, scenario,"FatJet_phi1_nominal", outTreeFile, FatJet_phi1_nominal)
    systTree.branchTreesSysts(trees, scenario,"FatJet_M1_nominal", outTreeFile, FatJet_M1_nominal)
    ...
    if(isMC):
        ...
        #h_cutFlow.GetXaxis().SetBinLabel(3,"n H/Z > 0")
        h_cutFlow.GetXaxis().SetBinLabel(3,"nTop_wp90 > 0")
    ...
    for i in range(tree.GetEntries()):
        ...
        #selected = list(filter(lambda x: x.particleNetMD_Xbb>=0.88,fatjets))
        #if(len(selected)<=0): continue # print(i,selected)
        #h_cutFlow.Fill("n H/Z > 0",1)
        ...
        #nZ_medium_nominal = len(list(filter(lambda x:(x.msoftdrop>=60) and (x.msoftdrop<105) and x.particleNetMD_Xbb>=0.88,fatjets)))
        if fatjets[loosefatjets[0].index].particleNetMD_Xbb<0.88: AK8_region_nominal[0]=0
        elif fatjets[loosefatjets[0].index].particleNetMD_Xbb>=0.985: AK8_region_nominal[0]=2
        else: AK8_region_nominal[0]=1
        FatJet_pt1_nominal[0]=fatjets[loosefatjets[0].index].pt
        FatJet_eta1_nominal[0]=fatjets[loosefatjets[0].index].eta
        FatJet_phi1_nominal[0]=fatjets[loosefatjets[0].index].phi
        #FatJet_E1_nominal[0]=fatjets[loosefatjets[0].index].E
        FatJet_M1_nominal[0]=fatjets[loosefatjets[0].index].msoftdrop
        ...
        if(sample.year == 2018 and not passesMETHEMVeto):
            if(not isMC and chain.run > 319077.):
                ...
                w_nominal_nominal[0] *= 1#0.354 da cambiare!!!
        ...
        if(len(all_coll_0p5)): top_region_nominal[0]=1
        else: top_region_nominal[0]=0
        ...
        if len(all_coll_wp90)>0:
            ...
            #if((HLT.Mu50) or (HLT.IsoMu24) or (HLT.Ele27_WPTight_Gsf) or (HLT.Photon200) or (HLT.AK8PFJet360_TrimMass30)):h_cutFlow.Fill("Trig_sel",1)
