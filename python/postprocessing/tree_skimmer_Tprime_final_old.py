...
def reco(scenario, isMC, addPDF, training):
    ...
    for i in range(tree.GetEntries()):
        ...
        if(len(all_coll_wp90)==0): continue
        ...
        goodfatjets = list(filter(lambda x: deltaR(x.eta,x.phi,Top_eta_nominal[0],Top_phi_nominal[0])>1.2 and (x.particleNetMD_Xbb>=0.88 or (x.msoftdrop>=60 and x.msoftdrop<=140)),fatjets))
        if len(goodfatjets)==0: continue
        goodfatjets.sort(key=lambda x:x.particleNetMD_Xbb,reverse=True)
        if goodfatjets[0].particleNetMD_Xbb<0.88: AK8_region_nominal[0]=0
        elif goodfatjets[0].particleNetMD_Xbb>=0.985: AK8_region_nominal[0]=2        
        ...
        if not "Data" in sample.label:
            genpart = Collection(event,"GenPart")
            higgs = list(filter(lambda x: x.pdgId==25,genpart))
            if len(higgs)>0:
                if (deltaR(goodfatjets[0].eta,goodfatjets[0].phi,higgs[-1].eta,higgs[-1].phi)<0.8):
                    FatJet_MC_nominal[0]=1
                else:
                    FatJet_MC_nominal[0]=0
            else:FatJet_MC_nominal[0]=0    
        ...
        other_bjet = list(filter(lambda x: alljets.index(x)!=int(lista[0].bjet_index) and deltaR(goodfatjets[0].eta,goodfatjets[0].phi,x.eta,x.phi)>1.2, bjets))
        ...
        dR_jet_AK8_nominal[0] = deltaR(goodfatjets[0].eta,goodfatjets[0].phi,Jet_phi_nominal[0],Jet_eta_nominal[0])
